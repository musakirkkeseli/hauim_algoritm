...
public class XAlgoHAUIMSA {
    ...
    Map<Integer, Integer> mapItemToAUUB0;
    List<Integer> auubPattern;// the items which has auub value more than minUtil
    ...
    class ChroNode implements Comparable<ChroNode> {
        ...
        public void calculateFitness(int k, List<Integer> templist) {
            ...
            double totalUtility = 0.0; // tüm destekleyen işlemler için toplam fayda
            for (int m = 0; m < templist.size(); m++) {
                int p = templist.get(m); // transaction index
                int i = 0, j = 0, q = 0, temp = 0;
                int sum = 0;

                while (q < database.get(p).size() && i < this.chromosome.length()) {
                    if (this.chromosome.get(i)) {
                        if (database.get(p).get(q).item == auubPattern.get(i)) {
                ...
                if (temp == k) {
                    totalUtility += sum; // örüntü bu işlemde tam bulunuyorsa katkı ekle
                }
            }
            this.fitness = totalUtility / (double) k; // *** HAUIM: ortalama fayda ***
        }

        @Override
        public int compareTo(ChroNode o) {
            return Double.compare(o.fitness, this.fitness); // büyükten küçüğe sırala
        }
    }
    ...
    List<Double> percentage = new ArrayList<Double>();// the portation of auub value of each
                                                      // 1-HAUUBIs in sum of auub value
    ...
    public void runAlgorithm(String input, String output, int minUtility) throws IOException {
        ...
        // We create a map to store the AUUB of each item
        mapItemToAUUB = new HashMap<Integer, Integer>();
        mapItemToAUUB0 = new HashMap<Integer, Integer>();

        // We scan the database a first time to calculate the AUUB of each item.
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                String itemUtilities[] = split[2].split(" ");

				int maxUtility = 0;

                for (int i = 0; i < items.length; i++) {
					if (maxUtility > Integer.parseInt(itemUtilities[i])) {
						maxUtility = Integer.parseInt(itemUtilities[i]);
					}
				}
                ...
                for (int i = 0; i < items.length; i++) {
                    ...
                    Integer auub0 = mapItemToAUUB0.get(item);
                    // add the utility of the item in the current transaction to
                    // its auub
                    auub = (auub == null) ? maxUtility : auub + maxUtility;
                    auub0 = (auub0 == null) ? maxUtility : auub0 + maxUtility;
                    mapItemToAUUB.put(item, auub);
                    mapItemToAUUB0.put(item, auub0);
                }
            }
        } catch (Exception e) {
        ...
        // OF 1-ITEMSETS HAVING AUUB >= minutil (promising items)
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for (int i = 0; i < items.length; i++) {
                    ...
                    if (mapItemToAUUB.get(pair.item) >= minUtility) {
                        // add it
                        revisedTransaction.add(pair);
                        pattern.add(pair.item);
                    } else {
                        mapItemToAUUB0.remove(pair.item);
                    }
                }
                // Copy the transaction into database but
                // without items with AUUB < minutility
        ...
        auubPattern = new ArrayList<Integer>(mapItemToAUUB0.keySet());
        Collections.sort(auubPattern);

        Items = new ArrayList<Item>();

        for (Integer tempitem : auubPattern) {
    ...
    private void pop_Init(int minUtility)//
    {
        ...
        // initial percentage according to the auub value of 1-HAUUBIs
        percentage = roulettePercent();

        while (i < pop_size) {
            // generate a chromosome
            ChroNode tempNode = new ChroNode(auubPattern.size());
            // initial chromosome
            j = 0;
            // k is the count of 1 in particle
            k = (int) (Math.random() * auubPattern.size());
    ...
    private List<ChroNode> neighbor(int minUtility) {
        ...
        for (int i = 0; i < pop_size; i++) {
            int temp = (int) (Math.random() * auubPattern.size());
    ...
    private List<Double> roulettePercent() {
        ...
        // calculate the sum of auub value of each 1-HAUUBIs
        for (i = 0; i < auubPattern.size(); i++) {
            sum = sum + mapItemToAUUB.get(auubPattern.get(i));
        }
        // calculate the portation of auub value of each item in sum
        for (i = 0; i < auubPattern.size(); i++) {
            tempSum = tempSum + mapItemToAUUB.get(auubPattern.get(i));
     ...
     * 
     * @param percentage the portation of auub value of each 1-HAUUBIs in sum of
     *                   auub
     *                   value
    ...
    public void printStats() {
        ...
        System.out.println(" High-average-utility itemsets count : " + huiSets.size());
        System.out.println("===================================================");
    }
}
